...
public class DriveTrain extends SubsystemBase {
  ...
  private final CANSparkMax m_frontRightMotor =
      new CANSparkMax(FRONT_RIGHT_MOTOR_PORT, MotorType.kBrushless);

  private final CANSparkMax m_frontLeftMotor =
      new CANSparkMax(BACK_LEFT_MOTOR_PORT, MotorType.kBrushless);
  ...
  private final DifferentialDrive m_drive =
      new DifferentialDrive(m_frontLeftMotor, m_frontRightMotor); // Main robot drive

  // Trigger for full speed
  public final Trigger atFullSpeed =
      new Trigger(() -> MathUtil.isNear(0.5, m_frontLeftMotor.get(), 0.05));
  ...
  public Command runArcadeDrive(DoubleSupplier forward, DoubleSupplier rotation) {
    ...
    return run(
        () -> {
          m_drive.arcadeDrive(
              forward.getAsDouble() * Constants.MAX_SPEED,
              rotation.getAsDouble() * Constants.MAX_SPEED);
        });
  }
