...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    controller
        ...
        .whileTrue(
            Commands.parallel(
                shooter.setVelocityState(ShooterConstants.ShooterState.IDLE), // turn off shooter
                // move pivot to intake position, run intake and index
                pivot.setPositionState(PivotState.INTAKE),
                intake.setVelocityState(IntakeState.INTAKE),
                index.setVelocityState(IndexState.INTAKE)))
        .onFalse(
            Commands.parallel(
                // turn off intake mechanism when trigger is released
                intake.setVelocityState(IntakeState.IDLE),
                index.setVelocityState(IndexState.IDLE)));
    // shut off intake mechanism automatically when note enters intake
    intake.noteInIntake.onTrue(
        Commands.parallel(
            intake.setVelocityState(IntakeState.IDLE), index.setVelocityState(IndexState.IDLE)));

    // manually override pivot angle
    controller
        .povUp()
        .onTrue(pivot.setPositionState(PivotState.MANUAL_OVERRIDE))
        .whileTrue(pivot.setPivotPctCommand(0.05));
    ...
    controller
        ...
        .whileTrue(
            Commands.parallel(
                pivot.setPositionState(PivotState.INTAKE),
                index.setVelocityState(IndexState.EXTAKE),
                intake.setVelocityState(IntakeState.EXTAKE)))
        .onFalse(
            Commands.parallel(
                index.setVelocityState(IndexState.IDLE),
                intake.setVelocityState(IntakeState.IDLE)));
    ...
    controller
        ...
        .whileTrue(
            Commands.parallel(
                index.setVelocityState(IndexState.EXTAKE),
                shooter.setVelocityState(ShooterState.EXTAKE)))
        .onFalse(
            Commands.parallel(
                index.setVelocityState(IndexState.IDLE),
                intake.setVelocityState(IntakeState.IDLE)));
    ...
    controller.y().onTrue(shooter.setVelocityState(ShooterConstants.ShooterState.IDLE));
    ...
    controller
        ...
        .onTrue(
            shooter.run(
                () -> {
                  switch (pivot.getPivotState()) {
                    case AMP -> shooter.setVelocityState(ShooterState.AMP);
                    case SHUTTLE -> shooter.setVelocityState(ShooterState.SHUTTLE);
                    case EXTAKE -> shooter.setVelocityState(ShooterState.EXTAKE);
                    case SUBWOOFER -> shooter.setVelocityState(ShooterState.SPEAKER);
                    case INTAKE -> shooter.setVelocityState(ShooterState.EXTAKE);
                    case MANUAL_OVERRIDE -> shooter.setVelocityState(ShooterState.SPEAKER);
                    case AIMING -> shooter.setVelocityState(ShooterState.SPEAKER);
                  }
                }))
        .onFalse(
            index
                .setVelocityState(
                    pivot.getPivotState() == PivotState.AMP ? IndexState.AMP : IndexState.SPEAKER)
